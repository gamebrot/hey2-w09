...
__cfg80211_rdev_from_attrs(struct net *netns, struct nlattr **attrs)
{
	...
	if (attrs[NL80211_ATTR_IFTYPE] && nla_get_u32(attrs[NL80211_ATTR_IFTYPE]) == NL80211_IFTYPE_AP) {
		pr_err("rdev=%p\n", rdev);
	}
			       ...
			       const u8 *ie, size_t ie_len, bool from_ap)
{
	...
	printk(KERN_ERR "nl80211_send_disconnected %d ie len %d enter", reason, (u32)ie_len);
#endif
...
#ifdef CONFIG_HN_WIFI
	printk(KERN_ERR "nl80211_send_disconnected %d exit", reason);
#endif
	genlmsg_multicast_netns(&nl80211_fam, wiphy_net(&rdev->wiphy), msg, 0,
				NL80211_MCGRP_MLME, GFP_KERNEL);
	return;
