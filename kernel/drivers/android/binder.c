...
#define CGROUP_ID_BACKGROUND 3
#define CGROUP_ID_SYSTEM_BACKGROUND 7
#endif
 ...
 */
#define HN_DYNAMIC_VIP_BINDER 0
static bool test_dynamic_vip(struct task_struct *task, int type) {
	bool ret = false;
	trace_android_rvh_honor_test_dynamic_vip(task, type, &ret);
	return ret;
}
static void dynamic_vip_dequeue(struct task_struct *task, int type) {
	trace_android_rvh_honor_dynamic_vip_dequeue(task, type);
}
static void dynamic_vip_enqueue(struct task_struct *thread_task, int type, struct task_struct *from_task) {
	trace_android_rvh_honor_dynamic_vip_enqueue(thread_task, type, from_task);
}
static bool test_task_vip_depth(struct task_struct *task) {
	bool ret = false;
	trace_android_rvh_honor_test_task_vip_depth(task, &ret);
	return ret;
}
static bool test_task_vip(struct task_struct *task) {
	bool ret = false;
	trace_android_rvh_honor_test_task_vip(task, &ret);
	return ret;
}
#endif // CONFIG_HN_VIP_THREAD_BUILDIN
...
#ifdef CONFIG_HN_VIP_THREAD_BUILDIN
static inline void
binder_thread_check_and_set_dynamic_vip(struct binder_thread *thread,
					struct binder_thread *from)
{
	if (from && test_task_vip(from->task) &&
	    test_task_vip_depth(from->task) &&
	    !test_task_vip(thread->task) && binder_enable_fg_switch)
		dynamic_vip_enqueue(thread->task, HN_DYNAMIC_VIP_BINDER,
				    from->task);
}

static inline void
binder_thread_check_and_remove_dynamic_vip(struct binder_thread *thread)
{
	if (test_dynamic_vip(thread->task, HN_DYNAMIC_VIP_BINDER))
		dynamic_vip_dequeue(thread->task, HN_DYNAMIC_VIP_BINDER);
}
#endif
#endif
...
bool check_binder_calling_work(int called_pid)
{
	...
	hlist_for_each_entry(proc, &binder_procs, proc_node) {
		if (proc->pid == called_pid) {
			if(proc->is_frozen) {
				pr_info("check_binder_calling_work get binder_proc is frozen for pid:%d\n", called_pid);
				continue;
			}
...
static bool binder_from_bg_thread(struct binder_transaction *trans, int task_qos)
{
	...
	if (task) {
		css_set = task_css(task, cpuset_cgrp_id);
		css_ctl = task_css(task, cpu_cgrp_id);
		if ((css_set->id == CGROUP_ID_BACKGROUND) || (css_ctl->id == CGROUP_ID_BACKGROUND) ||
                    (css_set->id == CGROUP_ID_SYSTEM_BACKGROUND) ||
                    (css_ctl->id == CGROUP_ID_SYSTEM_BACKGROUND)) {
			return true;
...
static void check_async_binder_rt(struct binder_transaction *t,
	...
	if ((g_sysctl_async_binder_rt_switch == 1) && oneway &&
		...
		if (t != NULL && from_thread->task != NULL &&
		    ...
		    (to_proc != NULL && to_proc->tsk != NULL && strcmp(to_proc->tsk->comm, ASYNC_BINDER_RT_TO_PROC) == 0))) {
...
#ifdef CONFIG_HN_VIP_THREAD_BUILDIN
		do_enqueue =
			do_enqueue || (test_task_vip(current) && (!oneway) &&
				       binder_enable_fg_switch);
#endif
		if (do_enqueue) {
			/*
...
#endif
#ifdef CONFIG_HN_SET_ASYNC_BINDER_RT
		check_async_binder_rt(t, thread, target_proc, tr, tr->flags & TF_ONE_WAY);
#endif
	}
 ...
 retry_alloc:
	...
	if (reply) {
		...
		if (target_proc->is_frozen) {
			pr_info("Sync transaction while in frozen state, %d binder call %d\n",
...
#endif
			binder_restore_priority(thread->task, proc->default_priority);
#ifdef CONFIG_HN_SET_ASYNC_BINDER_RT
		}
#endif
	}
			       ...
			       struct binder_proc *target_proc)
{
	...
	if (frozen(target_proc->tsk) || freezing(target_proc->tsk)) {
		pr_info("binder_ioctl_freeze is frozen or freezing,  pid:%d\n", target_proc->pid);
		return 0;
	}
#endif
...
#ifdef CONFIG_KSTATE_COMMON
			if (frozen(target_proc->tsk) || freezing(target_proc->tsk)) {
				info->sync_recv |= target_proc->sync_recv | 0;
				info->async_recv |= target_proc->async_recv | 0;
				pr_info("binder_ioctl_get_freezer_info is frozen or freezing,  pid:%d\n", target_proc->pid);
				continue;
			}
#endif
