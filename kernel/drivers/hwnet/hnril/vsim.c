...
void vsim_tether_stats(uint64_t tether_rx_bytes, uint64_t tether_tx_bytes)
{
	if (unlikely(!spin_trylock_bh(&vsim_lock))) {
		return;
	}
	...
	pr_info("%s: vsim_ifname:%s, rx_bytes:%lu, tx_bytes:%lu",
		__func__, vsim_ifname, tether_rx_bytes, tether_tx_bytes);
...
EXPORT_SYMBOL_GPL(vsim_tether_stats);
