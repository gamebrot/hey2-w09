			     ...
			     size_t len, loff_t *ptr)
{
	...
	case FFS_READ_STRINGS:
		...
		if (ffs->state == FFS_READ_DESCRIPTORS) {
			pr_info("read descriptors start\n");
			...
			ret = len;
			pr_info("read descriptors end\n");
		} else {
			pr_info("read strings start\n");
			ret = __ffs_data_got_strings(ffs, data, len);
			if (unlikely(ret < 0))
				break;
			pr_info("read strings end\n");
...
static void ffs_user_copy_worker(struct work_struct *work)
{
	...
	//pr_info("%s %s enter: ret %d for %s", __func__, ffs->dev_name, ret, io_data->read ? "read" : "write");
	if (ffs && ffs->dev_name) {
		if (strcmp(ffs->dev_name, "diag") == 0) {
			if (ret > 0) {
				...
				pr_err("[USB_LOGS] %s state ret = %d invalid\n", io_data->read ? "read" : "write", ret);
			}
		}
	}
...
static ssize_t ffs_epfile_io(struct file *file, struct ffs_io_data *io_data)
{
	...
	if (epfile->ep != ep) {
		...
		if (!(io_data->read) && (strcmp(ffs->dev_name, "diag") == 0))
			pr_err("[USB_LOGS]diag write async queued %zd bytes on %s\n", data_len, epfile->name);
