		...
		struct xhci_transfer_event *event)
{
	...
	case COMP_STOPPED:
		xhci_warn(xhci, "Stopped on Transfer TRB for slot %u ep %u\n",
			 slot_id, ep_index);
		break;
	case COMP_STOPPED_LENGTH_INVALID:
		xhci_warn(xhci,
	...
	case COMP_STALL_ERROR:
		xhci_warn(xhci, "Stalled endpoint for slot %u ep %u\n", slot_id,
			 ep_index);
		status = -EPIPE;
		break;
	case COMP_SPLIT_TRANSACTION_ERROR:
		xhci_warn(xhci, "Split transaction error for slot %u ep %u\n",
			 slot_id, ep_index);
		status = -EPROTO;
		break;
	case COMP_USB_TRANSACTION_ERROR:
		xhci_warn(xhci, "Transfer error for slot %u ep %u on endpoint\n",
			 slot_id, ep_index);
		status = -EPROTO;
		break;
	case COMP_BABBLE_DETECTED_ERROR:
		xhci_warn(xhci, "Babble error for slot %u ep %u on endpoint\n",
			 slot_id, ep_index);
		status = -EOVERFLOW;
		break;
	...
	case COMP_RING_UNDERRUN:
		...
		goto cleanup;
	case COMP_RING_OVERRUN:
		xhci_warn(xhci, "overrun event on endpoint\n");
		if (!list_empty(&ep_ring->td_list))
