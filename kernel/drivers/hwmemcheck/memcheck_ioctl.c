...
static int process_do_command(const void *arg)
{
	...
	memcheck_info("IOCTL COMMAND id=%u,type=%u,timestamp=%llu,cmd=%d,fullThreshold=%d,rate=%d\n",
		cmd.id, cmd.type, cmd.timestamp, cmd.cmd, cmd.sampleInfo.fullThreshold, cmd.sampleInfo.rate);
