 ...
 *
 * unify kernel memory leak interface for adapt to different platform honor or mtk
 *
...
size_t get_mem_total(int type)
{
	...
	return honor_get_mem_total(type);
#else
...
{
#ifdef CONFIG_HONOR_PAGE_TRACE
	return honor_get_mem_detail(type, buf, len);
#else
...
{
#ifdef CONFIG_HONOR_PAGE_TRACE
	return honor_page_trace_on(type, name);
#else
...
{
#ifdef CONFIG_HONOR_PAGE_TRACE
	return honor_page_trace_off(type, name);
#else
...
{
#ifdef CONFIG_HONOR_PAGE_TRACE
	return honor_page_trace_open(type, subtype);
#else
...
{
#ifdef CONFIG_HONOR_PAGE_TRACE
	return honor_page_trace_close(type, subtype);
#else
...
{
#ifdef CONFIG_HONOR_PAGE_TRACE
	return honor_page_trace_read(type, info, len, subtype);
#else
...
{
#ifdef CONFIG_HONOR_PAGE_TRACE
	return honor_get_ion_by_pid(pid);
#elif defined(CONFIG_MM_PAGE_TRACE)
	return mm_get_ion_by_pid(pid);
#else
	return memcheck_get_ion_size_by_pid(pid);
#endif
}
