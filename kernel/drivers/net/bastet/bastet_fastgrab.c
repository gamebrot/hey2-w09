...
static bool detect_sock_state(const struct sock *pst_sock)
{
...
static void bst_fg_print_matched_info(uid_t sock_uid, __u16 src_port,
				      uint8_t uc_protocol_bitmap,
				      uint8_t uc_app_type, bool ipv4_flag)
{
	bastet_logd("BST_FG_CUSTOM:Find sock UID is %d", sock_uid);
	...
	bastet_logd("ipv4_flag is %d", ipv4_flag);
}

bool bst_fg_is_socket_set(const struct sock *sock, struct bst_fg_custom_info *custom_info)
{
	uint8_t idx;
	unsigned long sock_ino = sock_i_ino((struct sock *)sock);
	...
	for (idx = 0; idx < custom_info->socket_num; idx++) {
		if (sock == custom_info->socket_info[idx].sock_ptr) {
			if (sock_ino == custom_info->socket_info[idx].ino) {
				return true;
			}
 ...
 *
 * @*pst_sock: the object of socket
 * @uc_protocol_bitmap: bitmap of TCP or UDP.
 * @skb: the object of socket buffer
 * @dst_port: the destination port of the flow
 * @ipv4_flag: is ipv4 flow (otherwise ipv6 flow)
 *
 * start to process the custom request.
 */
void bst_fg_custom_process(const struct sock *pst_sock,
			   uint8_t uc_protocol_bitmap, uint16_t dst_port,
			   bool ipv4_flag, struct sk_buff *skb)
{
	uid_t sock_uid;
	uint8_t idx;
	uint8_t uc_app_num;
	...
	if (htons(dst_port) == DNS_PORT_NUM) {
		bst_set_high_prio_mark(skb, ipv4_flag);
		return;
	}

	sock_uid = sock_i_uid((struct sock *)pst_sock).val;
	if (!bst_fg_is_uid_valid(sock_uid))
		return;

	uc_app_num = bst_min(g_st_bastet_fg_custom.uc_app_num, BST_FG_MAX_CUSTOM_APP);
	for (idx = 0; idx < uc_app_num; idx++) {
		if (sock_uid == g_st_bastet_fg_custom.ast_custom_info[idx].uid) {
	...
	bst_set_high_prio_mark(skb, ipv4_flag);
	bst_fg_print_matched_info(sock_uid, pst_sock->sk_num,
				  uc_protocol_bitmap,
				  past_custom_info->uc_app_type, ipv4_flag);
}

void bst_set_high_prio_mark(struct sk_buff *skb, bool ipv4_flag)
{
	...
	if (is_ds_rnic(out)) {
		if (ipv4_flag) {
			...
			iph = skb_header_pointer(skb, noff, sizeof(_iph),
						 &_iph);
			if (!iph) {
			...
			ip6h = skb_header_pointer(skb, noff, sizeof(_ip6h),
						  &_ip6h);
			if (!ip6h) {
				bastet_loge("ip6h is NULL");
				return;
			}

			ip6h->priority = QCOM_DSCP_MARK >> 4;
			ip6h->flow_lbl[0] = (ip6h->flow_lbl[0] & 0xF) |
					    ((QCOM_DSCP_MARK & 0xF) << 4);
		}
	} else if (is_wifi_rnic(out)) {
		skb->mark = QCOM_WIFI_MARK;
	}
}
#endif
