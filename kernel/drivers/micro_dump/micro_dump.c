...
#endif
#ifdef CONFIG_HN_AEE_REBOOT_REASON
#include "log/hn_rb_reason.h"
#endif
...
#ifndef CONFIG_HN_MICRODUMP_DEFAULT_ENABLE
static bool g_microdump_enable = true;
#else
static bool g_microdump_enable = false;
#endif
#define MICRODUMP_ENABLE       "true"
#define MICRODUMP_DISABLE      "false"

struct global_exp_ctrl g_exp_ctrl;

struct fault_reason_data {
	const char *str;
	unsigned long type;
};

static const struct fault_reason_data g_fault_reason[] = {
	{"BUG",								SUB_TYPE_BUG},
...
static unsigned long is_undefine_cmd(void)
{
#ifndef CONFIG_MSTAR_CHIP
#ifdef CONFIG_HN_AEE_REBOOT_REASON
	struct hn_aee_reboot_reason *g_aee_reboot_reason;
	g_aee_reboot_reason =
		(struct hn_aee_reboot_reason *)get_hn_aee_rb_reason_buf();
	if (strstr(g_aee_reboot_reason->sreason_str, "undefine_cmd"))
		return SUB_TYPE_UNDEFINE_INSTR;
#endif
#endif
	return 0;
}
...
#ifdef CONFIG_HN_MICRODUMP_GKI
void get_fault_addr(void *ignore, struct pt_regs *regs, unsigned int esr, unsigned long addr, const char *msg)
{
	g_fault_addr = addr;
	MD_PRINT("set error addr: %016llx\n", g_fault_addr);
}
#endif
...
static int die_callback(struct notifier_block *self, unsigned long cmd, void *ptr)
{
	...
	if (!g_microdump_enable) {
		MD_PRINT("The function is disable!\n");
		return NOTIFY_DONE;
	}

	if (atomic_inc_and_test(&g_has_die)) {
		type = is_undefine_cmd();
		if (!type) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
			type = analyse_kernel_fault(dargs->str,
						    dargs->regs->sdei_ttbr1,
						    dargs->err, dargs->regs);
#else
			type = analyse_kernel_fault(
				dargs->str, dargs->regs->orig_addr_limit,
				dargs->err, dargs->regs);
#endif
		}
		micro_dump_reason_set(dargs->str, type, NULL);
		g_exp_ctrl.die_cpu = raw_smp_processor_id();
		MD_PRINT("die_callback, error type is: 0x%lx \n", type);

		if (dargs && dargs->regs) {
#ifdef CONFIG_HN_MICRODUMP_GKI
			g_exp_ctrl.addr = g_fault_addr;
#else
...
static ssize_t microdump_enable_show(struct kobject *kobj,
				struct kobj_attribute *attr,
				char *buf)
{
	const char *src = g_microdump_enable ? MICRODUMP_ENABLE : MICRODUMP_DISABLE;

	return strlen(strncpy(buf, src, strlen(src)));
}

static ssize_t microdump_enable_store(struct kobject *kobj,
				struct kobj_attribute *attr,
				const char *buf, size_t count)
{
	if (sysfs_streq(buf, MICRODUMP_ENABLE))
		g_microdump_enable = true;
	else if (sysfs_streq(buf, MICRODUMP_DISABLE))
		g_microdump_enable = false;

	return count;
}

static struct kobj_attribute microdump_enable_attr = {
	.attr = {
		.name = "enable",
		.mode = 0640,
	},
	.show = microdump_enable_show,
	.store = microdump_enable_store,
};

static struct attribute *sysfs_attrs[] = {
	&microdump_enable_attr.attr,
	NULL
};
...
static void enable_sysfs_init(void)
{
	int ret = 0;

	struct kobject *kobj = kobject_create_and_add("microdump", kernel_kobj);
	if (!kobj) {
		MD_PRINT("kobject create failed!\n");
		return;
	}

	ret = sysfs_create_group(kobj, &sysfs_attr_group);
	if (ret) {
		kobject_put(kobj);
		MD_PRINT("sysfs group create failed!\n");
	}
}
...
#ifdef CONFIG_HN_MICRODUMP_GKI
	ret = register_trace_android_rvh_die_kernel_fault(get_fault_addr, NULL);
	if (ret)
		MD_PRINT("register get_fault_addr vhook fault\n");
	ret = register_trace_android_rvh_do_undefinstr(microdump_instr_vh, NULL);
	if (ret)
		MD_PRINT("register microdump_instr vhook fault\n");
#endif
