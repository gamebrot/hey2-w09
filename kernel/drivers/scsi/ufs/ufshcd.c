...
static void set_bootdevice_wb_enable(struct ufs_hba *hba, u8 wb_enable)
{
	//update hba->wb_enabled
	trace_android_vh_ufs_check_int_errors(hba, false);
}

static void ufshcd_set_wb_permanent_disable(struct ufs_hba *hba)
{
	unsigned long flags;
	set_bootdevice_wb_enable(hba, 0);
...
static inline void ufshcd_wb_config(struct ufs_hba *hba)
{
	...
	if (ret) {
		...
		set_bootdevice_wb_enable(hba, 1);
	}
#else
	ret = ufshcd_wb_ctrl(hba, true);
	if (ret)
