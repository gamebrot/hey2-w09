...
#define HHGKI_ENABLE_ZSWAPD_SHRINK_ITEMS 1
static atomic_t hhgki_zswapd_shrink_enable = ATOMIC_INIT(1);
...
static unsigned int zswapd_shrink_memcg(unsigned int nr_to_shrink)
{
	struct mem_cgroup_ext *memcg_ext = NULL;
	struct mem_cgroup *memcg = NULL;
	int i, ub_mem2zram_ratio;
	unsigned int nr_shrinked = 0;
	unsigned int nr_to_shrink_mcg = 0;
	...
	while ((memcg_ext = get_next_memcg_ext(memcg_ext))) {
		...
		for (i = 0; i < NR_INACTIVE_FILE; i++){
			memcg_anon_size += memcg_page_state_local(memcg, NR_LRU_BASE + i);
		}
...
static int hhgki_zswapd(void *p)
{
	...
	while (!kthread_should_stop()) {
		...
		if (hhgki_anon_size_is_low()) {
			low_anon = true;
			atomic64_inc(&stat->zswapd_lowanon);
			hh_print(HHLOG_INFO, "hhgki_zswapd hhgki_anon_size_is_low\n");
			goto do_eswap;
		}

		jiff_time = jiffies;
		if(atomic_read(&hhgki_zswapd_shrink_enable))
			zswapd_shrink();
		jiff_time = jiffies - jiff_time;
		if(jiff_time > atomic64_read(&stat->zswapd_shrink_maxtime))
			atomic64_set(&stat->zswapd_shrink_maxtime,jiff_time);
		hh_print(HHLOG_DEBUG, "HHGKI: shrink jiff_time  %llu\n", jiffies_to_msecs(jiff_time));
...
do_eswap:
		if (!hyperhold_reclaim_work_running() &&
			...
			if (curr_buffers < avail) {
				...
				atomic64_inc(&stat->zswapd_noswap);
		}
...
{
	ssize_t ret;
	unsigned long shrink_maxtime;
	struct hyperhold_stat *stat = hyperhold_get_stat_obj();
	if(!stat) {
		return  scnprintf(buf, PAGE_SIZE, "stat err\n");
	}
	shrink_maxtime = jiffies_to_msecs(atomic64_read(&stat->zswapd_shrink_maxtime));
	ret = scnprintf(buf, PAGE_SIZE,
			...
			"zswapd_low_anon_times:%llu\n"
			"zswapd_noswap_times:%llu\n" 
			"zswapd_shrink_maxtime:%lu\n",
			...
			atomic64_read(&stat->zswapd_lowanon),
			atomic64_read(&stat->zswapd_noswap),
			shrink_maxtime
			);
...
ssize_t hyperhold_zswapd_shrink_enable_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t len)
{
	int enable;

	if (sscanf(buf, "%d", &enable) !=
		HHGKI_ENABLE_ZSWAPD_SHRINK_ITEMS) {
		hh_print(HHLOG_ERR, "param invalid\n");
		return -EINVAL;
	}

	atomic_set(&hhgki_zswapd_shrink_enable, !!enable);
	return len;
}

ssize_t hyperhold_zswapd_shrink_enable_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = scnprintf(buf, PAGE_SIZE, "hhgki_zswapd_shrink: %s\n",
			atomic_read(&hhgki_zswapd_shrink_enable) ? "enable":"disable");
	return ret;
}
