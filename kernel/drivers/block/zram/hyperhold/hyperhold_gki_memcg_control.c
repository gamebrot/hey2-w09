...
static void mem_cgroup_alloc_callback(void *ignore, struct mem_cgroup *mcg)
{
	...
	hh_print(HHLOG_INFO,"mem_cgroup mem_cgroup_alloc_callback mcg_id=%d",mcg->id.id);
	return;
}
		...
		struct cgroup_subsys_state *css, struct mem_cgroup *mcg)
{
	...
	if (css_tryget_online(&mcg->css))
		...
		hh_print(HHLOG_ERR,"mem_cgroup_alloc_callback get css failed mcg_id=%d", mcg->id.id);
...
void memcg_ext_init(void)
{
	...
	register_trace_android_vh_mem_cgroup_alloc(mem_cgroup_alloc_callback, NULL);
