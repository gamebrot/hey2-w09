...
#ifdef CONFIG_BLK_SCH_BIGDATA
#define USER_IOM_BLK_GROUP	"io_blk_sched_group_scatter"
#endif
...
#ifdef CONFIG_BLK_SCH_BIGDATA
#define IOM_GRP_TIME_IDX_TO_MS(idx)	(1 << (idx + 3))
#endif
...
#define IOM_FS_INVAILD_THRESHOLD (1000000) //us
...
#ifdef CONFIG_BLK_SCH_BIGDATA
#define IOM_PROC_GROUP_BUF_LEN		(10 * TIME_DIV_NUM_G * IO_TP_NUM * 20)
#endif
...
#ifdef CONFIG_BLK_SCH_BIGDATA
enum iom_group_time_range {
	TIME_GRP_8 = 0,	/*<8ms*/
	TIME_GRP_16,	/*<16ms*/
	TIME_GRP_32,	/*<32ms*/
	TIME_GRP_64,	/*<64ms*/
	TIME_OVERSIZE_G,	/*>64ms*/
	TIME_DIV_NUM_G
};
#endif
...
#ifdef CONFIG_BLK_SCH_BIGDATA
	u64 iogroup_latency_stat[BLK_THROTL_TYPE_NR][TIME_DIV_NUM_G][IO_TP_NUM];
#endif
...
#ifdef CONFIG_BLK_SCH_BIGDATA
static inline u32 io_measure_latency_grp_time_index(u64 time_ms)
{
	u32 index = TIME_OVERSIZE_G;

	if (likely(time_ms <= IOM_GRP_TIME_IDX_TO_MS(TIME_GRP_8))) {
		index = TIME_GRP_8;
	} else if (time_ms <= IOM_GRP_TIME_IDX_TO_MS(TIME_GRP_16)) {
		index = TIME_GRP_16;
	} else if (likely(time_ms <= IOM_GRP_TIME_IDX_TO_MS(TIME_GRP_32))) {
		index = TIME_GRP_32;
	} else if (unlikely(time_ms > IOM_GRP_TIME_IDX_TO_MS(TIME_GRP_64))) {
		index = TIME_OVERSIZE_G;
	} else {
		index = TIME_GRP_64;
	}
	return index;
...
#ifdef CONFIG_BLK_SCH_BIGDATA
static int io_measure_blk_sched_group_proc_show(struct seq_file *m, void *v)
{
	struct io_measure_blk *iom_blk = (struct io_measure_blk *)m->private;
	u32 buf_len = IOM_PROC_GROUP_BUF_LEN;
	char *buf = NULL;
	int i, j, k;
	int offset = 0;
	...
	buf = kmalloc(IOM_PROC_GROUP_BUF_LEN, GFP_KERNEL);
	if (!buf)
		return -ENOMEM;
	memset(buf, 0, IOM_PROC_GROUP_BUF_LEN);

	//Latency title
	offset = scnprintf(buf, buf_len, "Latency[blk-scheduler]:\t");
	for (i = 0; i < TIME_DIV_NUM_G - 1; i++) {
		offset += scnprintf(buf + offset, buf_len - offset,
						"%lu\t", IOM_GRP_TIME_IDX_TO_MS(i));
	}
	offset += scnprintf(buf + offset, buf_len - offset, "oversize");

	//Latency stat
	for (i = 0; i < BLK_THROTL_TYPE_NR; i++) {
		offset += scnprintf(buf + offset, buf_len - offset,
						"\nIO[%lu]:\t", i);
		for (j = 0; j < TIME_DIV_NUM_G; j++) {
			for (k = 0; k < IO_TP_NUM; k++) {
				offset += scnprintf(buf + offset, buf_len - offset,
						"%lu%s", iom_blk->iogroup_latency_stat[i][j][k],
						k + 1 == IO_TP_NUM ? (j == TIME_OVERSIZE_G ? "" : "\t") : "/");
			}
		}
	}
...
static int io_measure_blk_sched_group_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, io_measure_blk_sched_group_proc_show, PDE_DATA(inode));
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static const struct proc_ops iom_blk_sched_group_proc_fops = {
	.proc_open = io_measure_blk_sched_group_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
#else
static const struct file_operations iom_blk_sched_group_proc_fops = {
	.open = io_measure_blk_sched_group_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
#ifdef CONFIG_BLK_SCH_BIGDATA
	proc_entry = proc_create_data(USER_IOM_BLK_GROUP, 0444, proc_root,
		&iom_blk_sched_group_proc_fops, iom_blk);
	if (proc_entry == NULL) {
		iom_pr_info("Create /proc/%s/%s fail.\n", USER_IOM_DIR, USER_IOM_BLK_GROUP);
		return -ENODEV;
	}
#endif
	return 0;
...
#ifdef CONFIG_BLK_SCH_BIGDATA
	if (current->pid == current->tgid || strstr(current->comm, "Render"))
		rq->need_statistics = true;
#endif
}
	...
	struct request_queue *q, struct request *rq)
{
	struct io_measure_blk *iom_blk = NULL;
#ifdef CONFIG_BLK_SCH_BIGDATA
	unsigned int grp_idx = BLK_THROTL_TYPE_NR;
	struct bio *biop = rq->bio;
...
#ifdef CONFIG_BLK_SCH_BIGDATA
	if(rq->need_statistics) {
		sch_time_idx = io_measure_latency_grp_time_index(IOM_NS_TO_MS(delta));
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
			if (biop && biop->bi_css) {
				blkcg = css_to_blkcg(biop->bi_css);
				if (blkcg != &blkcg_root)
#else
			if (biop && biop->bi_blkg && biop->bi_blkg->blkcg && biop->bi_blkg->blkcg != &blkcg_root) {
				blkcg = biop->bi_blkg->blkcg;
#endif
					grp_idx = blkcg->type < grp_idx ? blkcg->type : grp_idx ;
			}
			rcu_read_unlock();
		}

		if ((grp_idx < BLK_THROTL_TYPE_NR) && (io_type < IO_TP_NUM))
			iom_blk->iogroup_latency_stat[grp_idx][sch_time_idx][io_type]++;
	}
#endif
	//max io latency
	...
	struct inode *inode, loff_t offset, int bytes)
{
	...
	if ((now_us <= iom_item->start_time_us) || (iom_item->start_time_us == 0))
		return;
	delta = now_us - iom_item->start_time_us;
	if (unlikely(delta > IOM_FS_INVAILD_THRESHOLD)) {
		pr_info("[io_measure]io_measure_fs_dataread_end:invalid,latency:%uus.\n", delta);
		memset(iom_item, 0, sizeof(struct io_measure_fs_item));
		return;
	}
	...
	struct inode *inode, loff_t offset, int bytes)
{
	...
	delta = now_us - iom_item->start_time_us;
	if (unlikely(delta > IOM_FS_INVAILD_THRESHOLD)) {
		pr_info("[io_measure]io_measure_fs_datawrite_end:invalid,latency:%uus.\n", delta);
		memset(iom_item, 0, sizeof(struct io_measure_fs_item));
		return;
	}
...
#ifdef CONFIG_BLK_SCH_BIGDATA
	memset(iom_blk->iogroup_latency_stat, 0, sizeof(iom_blk->iogroup_latency_stat));
#endif
	// scsi measure reset
	iom_scsi = iom_ctrl->iom_scsi;
