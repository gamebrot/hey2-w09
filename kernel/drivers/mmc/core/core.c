...
void dsm_sdcard_set_value(int cmd, u32 err)
{
	struct mmc_host *host = dsm_sdcard_get_host();
	if (host)
		trace_android_vh_mmc_attach_sd(host, err, cmd);
}

void dsm_sdcard_report(int cmd, int err)
{
	struct mmc_host *host = dsm_sdcard_get_host();
	if ((host != NULL) && (cmd < DSM_SDCARD_CMD_MAX))
		trace_android_vh_mmc_attach_sd(host, err, DSM_SDCARD_CMD_MAX + cmd);
}
#endif
