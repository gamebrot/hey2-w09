...
#ifdef CONFIG_HONOR_SD_HEALTH_DETECT
MMC_DEV_ATTR(speed_class, "0x%08x\n", ((u8)(card->raw_ssr[2]>>24)));
...
int mmc_sd_get_csd(struct mmc_host *host, struct mmc_card *card)
{
	...
	if (!strcmp(mmc_hostname(host), "mmc1")) {
		dsm_sdcard_set_value(DSM_SDCARD_CMD9_R0, card->raw_csd[0]);
		dsm_sdcard_set_value(DSM_SDCARD_CMD9_R1, card->raw_csd[1]);
		dsm_sdcard_set_value(DSM_SDCARD_CMD9_R2, card->raw_csd[2]);
		dsm_sdcard_set_value(DSM_SDCARD_CMD9_R3, card->raw_csd[3]);
	}
	if (err) {
...
#ifndef CONFIG_HONOR_SDCARD_DSM
