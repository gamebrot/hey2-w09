...
static int _mmc_select_card(struct mmc_host *host, struct mmc_card *card)
{
	...
	if (!strcmp(mmc_hostname(host), "mmc1"))
		dsm_sdcard_set_value(DSM_SDCARD_CMD7, cmd.resp[0]);
	if (err) {
...
mmc_send_cxd_native(struct mmc_host *host, u32 arg, u32 *cxd, int opcode)
{
	...
	if (cmd.opcode == MMC_ALL_SEND_CID) {
		if (!strcmp(mmc_hostname(host), "mmc1")) {
			dsm_sdcard_set_value(DSM_SDCARD_CMD2_R0, cmd.resp[0]);
			dsm_sdcard_set_value(DSM_SDCARD_CMD2_R1, cmd.resp[1]);
			dsm_sdcard_set_value(DSM_SDCARD_CMD2_R2, cmd.resp[2]);
			dsm_sdcard_set_value(DSM_SDCARD_CMD2_R3, cmd.resp[3]);
		}
		if (err) {
			if (-ENOMEDIUM != err && -ETIMEDOUT != err
				&& !strcmp(mmc_hostname(host), "mmc1"))
