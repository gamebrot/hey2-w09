...
void set_sd_disk_health_status(struct gendisk *disk, char *status)
{
    ...
    printk(KERN_ERR "SD_HEALTH_DETECT mmc1:report sd messasge to userspace,status = %s\n",status);
}
...
unsigned int mmc_calculate_ioworkload_and_rwspeed(unsigned long long time, struct request *rqc, struct gendisk *disk, struct mmc_card *card)
{
   ...
   if(t_period >= (unsigned long long )PRT_TIME_PERIOD)
   {
       ...
       else
       {
          ...
          t_percent =(unsigned int)div64_u64(t_usage, t_period);//   real rw time/stat time
          t_percent_wr = (unsigned int)div64_u64(mmcqd_t_usage_wr, t_period);//    real write time/stat time-->to get write workload
