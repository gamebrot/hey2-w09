...
extern struct block_device *bdget(dev_t dev);
...
static struct hotfile_stat hf_stat;
...
static void hf_page_access_record(void *ignore, struct page *page)
{
	...
	if (HF_GET_FLAG(hf->enable_flag, HF_SYS_STAT) &&
		...
		if (task_blkcg(current)->type != BLK_THROTL_TA)
			return;
#endif
		type = HF_APP;
	} else {
 ...
 */
static struct inode *hf_get_inode(u32 ino, u32 dev)
{
...
static const struct file_operations hf_app_fn_list_proc_fops = {
	.open = hf_app_fn_list_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
#if  (LINUX_VERSION_CODE < KERNEL_VERSION(4, 20, 0))
static const struct file_operations hf_sys_fn_list_proc_fops = {
	.open = hf_sys_fn_list_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
#if  (LINUX_VERSION_CODE < KERNEL_VERSION(4, 20, 0))
static const struct file_operations hf_app_flist_proc_fops = {
	.open = hf_app_flist_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
	.write = hf_app_flist_proc_write,
};
#else
...
#if  (LINUX_VERSION_CODE < KERNEL_VERSION(4, 20, 0))
static const struct file_operations hf_sys_flist_proc_fops = {
	.open = hf_sys_flist_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
	.write = hf_sys_flist_proc_write,
};
#else
