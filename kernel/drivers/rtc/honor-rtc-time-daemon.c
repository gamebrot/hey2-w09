...
const struct rtc_class_ops* get_honor_rtc_time_daemon_rw_ops(
	...
	if (memcpy_s(honor_rtc_rw_ops_ptr, sizeof(struct rtc_class_ops), qpnp_rtc_rw_ops_ptr,
                sizeof(struct rtc_class_ops))) {
		pr_err("%s: memcpy qpnp_rtc_rw_ops_ptr failed !\n", __func__);
...
int honor_rtc_time_daemon_init(struct device *dev){
	...
	if(NULL == dev){
 	...
 	if (memcpy_s(&offset_info, sizeof(Offset_Info), smem->rtc_offset_info,
                     sizeof(Offset_Info))) {
		pr_err("%s: memcpy rtc_offset_info failed !\n", __func__);
		return -1;
        }
