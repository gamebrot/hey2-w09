...
enum bootfail_errorcode do_recovery(struct recovery_run_param *pparam)
{
	...
	if (pparam->padp->get_boot_slot != NULL) {
		pparam->padp->get_boot_slot(&last_boot_slot, &current_boot_slot);
		if (last_boot_slot != BOOT_SLOT_UNKNOWN && current_boot_slot != BOOT_SLOT_UNKNOWN &&
			last_boot_slot != current_boot_slot) {
			pparam->pparam->binfo.suggest_recovery_method = METHOD_DO_NOTHING;
		}
	}
#endif
