...
static int is_after_boot_success(void)
{
	enum bootfail_errorcode error_code;
	enum boot_stage bootstage;
	error_code = get_boot_stage(&bootstage);
	if (error_code != BF_OK) {
		print_err("get_boot_stage ret: %d!\n", error_code);
		return 0;
	}

	if (is_after_suss_stage(bootstage)) {
		return 1;
	}
...
static noinline void dump_stacktrace(char *pbuf, size_t buf_size)
{
	...
	for (i = 0; i < (int)com_len; i++)
		pbuf[i] = com_buf[i];
	//here need 2 bytes align, if not it will panic while copy tmp_buf to pbuf
	if (com_len & 0x1){
		pbuf[com_len] = ' ';
		com_len++;
	}
...
{
	if (is_after_boot_success()) {
		print_err("bootfail: already boot success!\n");
		return NOTIFY_DONE;
	}
