...
#endif
{
...
#ifdef CONFIG_HONOR_SCM_BUILDIN
	trace_android_rvh_do_shrink_slab(shrinker, &freeable);
#else
	...
	if (total_scan < 0) {
		...
		pr_err("nr=%ld, freeable=%ld, delta=%llu\n",
				nr, freeable, delta);
		total_scan = freeable;
		next_deferred = nr;
	} else
...
#ifdef CONFIG_SHRINKER_OPT
		if (!current_is_kswapd() && priority > 10)
			return 0;
#endif
...
unsigned long reclaim_pages_from_list_ex(struct list_head *page_list)
{
	struct scan_control sc = {
		.gfp_mask = GFP_KERNEL,
		.priority = DEF_PRIORITY,
...
#ifdef CONFIG_FILE_REFAULTS_OPT
		if (PageRefault_opt(page)) {
			ClearPageRefault_opt(page);
			continue;
		}
#endif
					...
					struct scan_control *sc)
{
	...
	trace_android_rvh_continue_reclaim(&sc->android_vendor_data1, &sc->nr_to_reclaim,
				&sc->nr_reclaimed, &continue_reclaim);
...
again:
	 ...
	 */
#ifdef CONFIG_HONOR_SCM_BUILDIN
	if (!cgroup_reclaim(sc) && !sc->file_is_tiny) {
#else
	if (!cgroup_reclaim(sc)) {
#endif
...
static void snapshot_refaults(struct mem_cgroup *target_memcg, pg_data_t *pgdat)
{
	...
	unsigned long refaults;
#ifdef CONFIG_HYPERHOLD_FILE_LRU
	struct lruvec *pgdat_lruvec;
	/*
	 * if CONFIG_HYPERHOLD_FILE_LRU open, anon used lru of per memcg
	 * but file use golbal lru.
	 */

	pgdat_lruvec = node_lruvec(pgdat);
	pgdat_lruvec->refaults[0] = lruvec_page_state(pgdat_lruvec, WORKINGSET_ACTIVATE_ANON);
	pgdat_lruvec->refaults[1] = lruvec_page_state(pgdat_lruvec, WORKINGSET_ACTIVATE_FILE);
#endif
					  ...
					  struct scan_control *sc)
{
	...
	trace_android_rvh_reclaim_sc(&sc->android_vendor_data1,
		&sc->nr_to_reclaim, sc->target_mem_cgroup, &file_is_tiny, &may_writepage);
...
static bool allow_direct_reclaim(pg_data_t *pgdat)
{
	...
	for (i = 0; i <= ZONE_NORMAL; i++) {
		...
		pfmemalloc_reserve += zone->_watermark[WMARK_MIN];
...
#ifdef CONFIG_HONOR_SCM_BUILDIN
	trace_android_rvh_dr_path(&sc.android_vendor_data1);
#endif
