...
#ifdef CONFIG_HONOR_SPF
#if ALLOC_SPLIT_PTLOCKS
static void wait_for_smp_sync(void *arg)
{
}
#endif
#endif	/* CONFIG_HONOR_SPF */
...
static int remap_pmd_range_thp(struct mm_struct *mm, pud_t *pud,
...
#ifdef CONFIG_HUGEPAGE_POOL
	if (unlikely(!(pgprot_val(prot) & PMD_TABLE_BIT)))
		return remap_pmd_range_thp(mm, pud, pmd, addr, end,
...
static vm_fault_t do_fault_around(struct vm_fault *vmf)
{
	...
	trace_android_rvh_do_fault_around(&nr_pages);
#endif
...
#ifdef CONFIG_SWAP_PAGE_STAT
			ret = do_page_fault_stat(do_wp_page, vmf, &swap_page_stat);
#else
			ret = do_wp_page(vmf);
#endif
