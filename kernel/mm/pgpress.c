...
static unsigned int current_fps = MIN_FRAME_RATE;
...
EXPORT_SYMBOL(pgpress_invoke_fps_chg_callback);
unsigned int get_current_fresh_rate(void)
{
	return current_fps;
}
...
void refault_monitor(void)
{
	...
	unsigned long frame_msecs_thres;
	unsigned int frame_ticks_thres;
	unsigned int percent_quota = PERCENT_HUNDRED;
	if (!g_feature_on)
		return;
	spin_lock(&g_pglock);
	if (!g_pgpress.quota)
	...
	if (g_pgpress.pgpress >= default_pgpress_level &&
		...
		g_pgpress.pgpress = 0;
	frame_msecs_thres = THRES_FRAME_NUM * MS_PER_S / get_current_fresh_rate();
	frame_ticks_thres = msecs_to_jiffies(frame_msecs_thres);
	if (time_after(jiffies, g_pgpress.start_time + frame_ticks_thres)) {
		percent_quota = PERCENT_HUNDRED - (PERCENT_HUNDRED * g_pgpress.quota) /
				g_pgpress.quota_source;
		g_pgpress.last_quota_left = g_pgpress.quota;
		goto calc_pgresss;
	}

	/* when quota is zero, update pgpress and reinit */
	if (!g_pgpress.quota) {
		g_pgpress.last_quota_left = 0;
calc_pgresss:
		time = jiffies_to_msecs(jiffies - g_pgpress.start_time);
		if (likely(time > 0)) {
			g_pgpress.last_frame_time = frame_msecs_thres;
			g_pgpress.last_quota_source = g_pgpress.quota_source;
			g_pgpress.last_time = time;
			g_pgpress.pgpress = (percent_quota * default_pgpress_level *
						PG_S_PER_MS / time) / PERCENT_HUNDRED;
		}
init:
		/* init quota = (100/quoota_ratio)% * file and record the init time */
		g_pgpress.quota = pgfile / quota_ratio;
		g_pgpress.start_time = jiffies;
		g_pgpress.quota_source = g_pgpress.quota;
	}
...
{
	seq_printf(m, "file: %lu free: %lu pgpress:%lu%% pgpress_time:%lu pgpress_quota:%lu pgpress_quota_left:%lu pgpress_frame_time:%u\n",
				PAGETOKB(global_node_page_state(NR_ACTIVE_FILE) +
				global_node_page_state(NR_INACTIVE_FILE)),
				PAGETOKB(global_zone_page_state(NR_FREE_PAGES)),
				g_pgpress.pgpress,
				g_pgpress.last_time,
				g_pgpress.last_quota_source,
				g_pgpress.last_quota_left,
				g_pgpress.last_frame_time);
	return 0;
}
