...
#ifdef CONFIG_CSS_RELEASE_DEBUG
...
static void percpu_ref_call_confirm_rcu(struct rcu_head *rcu)
{
	...
	trace_printk(
		"percpu confirm rcu %px: Callers:(%lx<-%lx<-%lx<-%lx<-%lx<-%lx<-%lx)\n",
		ref, addr[0], addr[1], addr[2], addr[3], addr[4], addr[5],
		addr[6]);
#endif
				 ...
				 percpu_ref_func_t *confirm_kill)
{
	...
	trace_printk(
		"percpu kill %px: Callers:(%lx<-%lx<-%lx<-%lx<-%lx<-%lx<-%lx)\n",
		ref, addr[0], addr[1], addr[2], addr[3], addr[4], addr[5],
		addr[6]);
#endif
