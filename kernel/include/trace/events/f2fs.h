...
DECLARE_EVENT_CLASS(f2fs__sis_inode,

	TP_PROTO(struct inode *inode, struct inode *h_inode),

	TP_ARGS(inode, h_inode),
	...
	TP_STRUCT__entry(
		...
		__field(loff_t, size)
		__field(unsigned int, nlink)
		__field(ino_t,  h_inode_ino)
		__field(unsigned int, h_inode_nlink)
	),
		...
		__entry->nlink  = inode->i_nlink;
		__entry->size   = inode->i_size;
		__entry->h_inode_ino  = h_inode->i_ino;
		__entry->h_inode_nlink    = h_inode->i_nlink;
	),

	TP_printk("dev = (%d,%d), ino = %lu, pino = %lu, "
		"i_size = %lld, i_nlink = %u, "
		"h_inode = %lu, h_inode i_nlink = %u",
		show_dev_ino(__entry),
		(unsigned long)__entry->pino,
		__entry->size,
		(unsigned int)__entry->nlink,
		(unsigned long)__entry->h_inode_ino,
		(unsigned int)__entry->h_inode_nlink)
);

DEFINE_EVENT(f2fs__sis_inode, f2fs_sis_ioc_trunc_file,
...
DEFINE_EVENT(f2fs__sis_inode, f2fs_ioc_sis_dedup_file,
...
DEFINE_EVENT(f2fs__sis_inode, f2fs_sis_recover_inode,
...
DEFINE_EVENT(f2fs__sis_inode, f2fs_sis_recover_fail,
...
DEFINE_EVENT(f2fs__sis_inode, f2fs_sis_dec_hidden_link,
...
DECLARE_EVENT_CLASS(f2fs__sis_map,
	...
	TP_printk("dev = (%d,%d), outer ino = %lu, i_size = %lld, "
		"map to h_inode ino = %lu, h_inode i_nlink = %u",
		show_dev_ino(__entry),
		__entry->size,
		(unsigned long)__entry->h_inode_ino,
		(unsigned int)__entry->h_inode_nlink)
...
DEFINE_EVENT(f2fs__sis_map, f2fs_sis_map_readpage,
...
DEFINE_EVENT(f2fs__sis_map, f2fs_sis_map_blocks,
