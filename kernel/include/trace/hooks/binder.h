...
DECLARE_RESTRICTED_HOOK(android_rvh_honor_test_dynamic_vip,
	TP_PROTO(struct task_struct *task, int type, bool *ret),
	TP_ARGS(task, type, ret), 1);

DECLARE_RESTRICTED_HOOK(android_rvh_honor_dynamic_vip_dequeue,
	TP_PROTO(struct task_struct *task, int type),
	TP_ARGS(task, type), 1);

DECLARE_RESTRICTED_HOOK(android_rvh_honor_dynamic_vip_enqueue,
	TP_PROTO(struct task_struct *thread_task, int type, struct task_struct *from_task),
	TP_ARGS(thread_task, type, from_task), 1);

DECLARE_RESTRICTED_HOOK(android_rvh_honor_test_task_vip_depth,
