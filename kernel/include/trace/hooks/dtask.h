...
struct rwsem_waiter;
DECLARE_HOOK(android_vh_honor_mutex_vip_set,
	TP_PROTO(struct mutex *lock),
	TP_ARGS(lock));
DECLARE_HOOK(android_vh_honor_rwsem_list_add,
	TP_PROTO(struct rwsem_waiter *waiter, struct rw_semaphore *sem, bool *already_on_list),
	TP_ARGS(waiter, sem ,already_on_list));
DECLARE_HOOK(android_vh_honor_rwsem_vip_set,
	TP_PROTO(struct rwsem_waiter *waiter, struct rw_semaphore *sem),
	TP_ARGS(waiter, sem));
