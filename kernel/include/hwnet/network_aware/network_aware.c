...
{
	int i;
	int cur_mode = aware_net_ctrl.mode;
	...
	if (!is_bg_limit_enabled() || cur_mode == 0)
		return;

	cur_time = jiffies;
	/* mode 7: bg limite dynamically */
	if (cur_mode == AWARE_NET_CTRL_MODE_FIRSE_LEVEL) {
		spin_lock(&(netinfo.fg_lock));
		for (i = 0; i < MAX_FG_NET_STAT; i++) {
		...
		sleep_long = aware_net_bg_ctrl_policy[cur_mode - 1].sleep_long;
	} else if (cur_mode <= AWARE_NET_CTRL_MODE_SECOND_LEVEL) {
		sleep_long = aware_net_bg_ctrl_policy[cur_mode - 1].sleep_long;
	}
