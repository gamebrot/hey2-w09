...
/* when modify wifi_ifaces array, plz modify the WIFI_IFACE_NUM simultaneously */
#define WIFI_IFACE_NUM 1
static const char wifi_ifaces[WIFI_IFACE_NUM][IFNAMSIZ]
...
bool is_wifi_rnic(const struct net_device *dev)
{
	int i;

	if (!dev)
		return false;
	for (i = 0; i < WIFI_IFACE_NUM; ++i) {
		if (ifname_equal(dev->name, sizeof(dev->name),
			wifi_ifaces[i], sizeof(wifi_ifaces[i])))
			return true;
	}
	return false;
}
