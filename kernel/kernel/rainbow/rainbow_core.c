...
/* Design for GKI version to record prb meta data */
...
#ifndef CONFIG_RAINBOW_GKI
static void rb_save_regin_log_info(struct rb_region_log_info *log_info,
	char *(*get_addr)(void), u32 (*get_len)(void))
{
	char *info_addr = NULL;
	uint64_t info_size = 0;
	info_addr = get_addr();
	info_size = (uint64_t)get_len();
	if (!info_addr || !info_size) {
		pr_err("Unable to find info_addr or info_size by kallsyms\n");
		return;
	}
	log_info->virt_addr = (uintptr_t)(info_addr);
	log_info->phys_addr = virt_to_phys(info_addr);
	log_info->size = info_size;
	log_info->magic = RB_REGION_LOG_MAGIC_VALID;
}

static void rb_save_kmsg_meta(struct rb_kmsg_meta_data *meta)
{
#ifdef CONFIG_PRINTK_META
	meta->prb_phys_addr = virt_to_phys((void *)get_printk_ring_buf_addr());
	meta->descs_phys_addr = virt_to_phys((void *)get_prb_descs_addr());
	meta->infos_phys_addr = virt_to_phys((void *)get_prb_infos_addr());
	meta->prb_size = printk_ring_buf_size_get();
	meta->magic = RB_REGION_LOG_MAGIC_VALID;
#else
	meta->magic = RB_REGION_LOG_MAGIC_INVALID;
#endif
}
#endif
...
static void rb_kernel_log_buf_reg(void)
{
	...
	/* rainbow module will load in kernel img which earlier
	 * than dynamic module load for non-GKI version. Recoring
	 * prb meta data two times:
	 * 1. kernel rainbow module init (earlier)
	 * 2. vendor_boot qcom_logbuf_vh.ko (later)
	 * */
#ifndef CONFIG_RAINBOW_GKI
	rb_save_regin_log_info(&g_rb_header->kernel_log, log_buf_addr_get, log_buf_len_get);
	rb_save_kmsg_meta(&g_rb_header->kmsg_meta);
#endif
}
