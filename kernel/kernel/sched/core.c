...
#ifdef CONFIG_CFS_BANDWIDTH_DATA_STAT
static int tg_reset_cfs_running_pct(struct task_group *tg, long reset_val)
{
	struct cfs_bandwidth *cfs_b = &tg->cfs_bandwidth;
	...
	raw_spin_lock_irq(&cfs_b->lock);
	memset(cfs_b->running_pct, 0, RUNNING_PCT_NUM * sizeof(unsigned int));
	raw_spin_unlock_irq(&cfs_b->lock);

	return 0;
}

static u64 cpu_cfs_reset_read_u64(struct cgroup_subsys_state *css,
				   struct cftype *cft)
{
	return 0;
}

static int cpu_cfs_reset_write_u64(struct cgroup_subsys_state *css,
				    struct cftype *cftype, u64 reset_val)
{
	return tg_reset_cfs_running_pct(css_tg(css), reset_val);
}

static int cpu_running_pct_show(struct seq_file *sf, struct cfs_bandwidth *cfs_b)
{
	int i, j = 0;

	seq_printf(sf, "CFS Group Bandwidth  Running Time Percentage Data : \n");
	for (i = 0; i < 10; i++) {
		seq_printf(sf, "Index: ");
		for (j = 0; j < 10; j++) {
			seq_printf(sf, "%16d", i * 10 + j);
		}
		seq_printf(sf, "\n");
		seq_printf(sf, "Value: ");
		for (j = 0; j < 10; j++) {
			seq_printf(sf, "%16d", cfs_b->running_pct[i * 10 + j]);
		}
		seq_printf(sf, "\n");
	}
...
static struct cftype cpu_legacy_files[] = {
	...
	{
		.name = "reset_stat",
		.read_u64 = cpu_cfs_reset_read_u64,
		.write_u64 = cpu_cfs_reset_write_u64,
	},
#endif
#endif
#ifdef CONFIG_RT_GROUP_SCHED
