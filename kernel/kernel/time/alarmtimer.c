...
#ifdef CONFIG_HONOR_RTC_WAKEUP_DEBUG
...
#else
#ifdef CONFIG_HONOR_RTC_WAKEUP_DEBUG
	pr_info("[alarmtimer info]%s: comm:%s pid:%d exp:%llu func:%ps\n", __func__,
...
#ifdef CONFIG_HONOR_RTC_WAKEUP_DEBUG
	if(alarm_debug & 0x1){
		pr_info("[alarmtimer info]%s: type=%d, func=%ps, exp:%llu\n", __func__,
...
static int alarmtimer_suspend(struct device *dev)
{
	...
	if (min_timer != NULL){
#ifdef CONFIG_HONOR_DUBAI_COMMON
		pr_info("[alarmtimer info]%s: type=%d, comm:%s pid:%d, func=%ps, min:%llu, exp:%llu\n",
				__func__, min_timer->type, min_timer->timer.comm, min_timer->timer.pid,
				min_timer->function, ktime_to_ns(min),
				ktime_to_ms(min_timer->node.expires));
#else
		pr_info("[alarmtimer info]%s: type=%d, func=%ps, min:%llu, exp:%llu\n",
				__func__, min_timer->type, min_timer->function, ktime_to_ns(min),
				ktime_to_ms(min_timer->node.expires));
#endif
		min_timer = NULL;
	}
	...
	if (ret < 0)
		...
		if ((kernelAlarmtimerHook != NULL) && (rtc_alarm != NULL)) {
			kernelAlarmtimerHook(rtc_alarm->timer.comm, rtc_alarm->timer.pid);
		}
		rtc_alarm = NULL;
	}
#endif
