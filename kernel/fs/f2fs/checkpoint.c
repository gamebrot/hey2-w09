...
static int recover_orphan_inode(struct f2fs_sb_info *sbi, nid_t ino)
{
	...
	if (is_inode_flag_set(inode, FI_RECOVER_INODE)) {
		f2fs_notice(sbi, "[sis]: recover orphan: ino[%lu] set recovering, flags[%lu]",
				ino, F2FS_I(inode)->flags);
		f2fs_bug_on(sbi, is_inode_flag_set(inode, FI_BEING_SIS));
		err = f2fs_sis_inode_trunc(inode, FI_RECOVER_INODE);
		iput(inode);
		return err;
	}

	if (is_inode_flag_set(inode, FI_BEING_SIS)) {
		f2fs_notice(sbi, "[sis]: recover orphan: ino[%lu] set changing, flags[%lu]",
				ino, F2FS_I(inode)->flags);
		err = f2fs_sis_inode_trunc(inode, FI_BEING_SIS);
		iput(inode);
		return err;
	}
#endif
