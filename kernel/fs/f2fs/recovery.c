...
static void recover_inline_flags(struct inode *inode, struct f2fs_inode *ri)
{
	...
	if (!((le32_to_cpu(ri->i_flags) & F2FS_COMPR_FL) ||
		(ri->i_inline & F2FS_COMPRESS_RELEASED)) &&
		is_inode_flag_set(inode, FI_COMPRESS_RELEASED)) {
		clear_inode_flag(inode, FI_COMPRESS_RELEASED);
		f2fs_compress_info("clear compress release flag, ino %lu, comp %d,"
			" rflags %lu, rel %u", inode->i_ino, f2fs_compressed_file(inode),
			le32_to_cpu(ri->i_flags), (ri->i_inline & F2FS_COMPRESS_RELEASED));
	}
#endif
}
