...
static void f2fs_sis_dec_hidden_link(struct inode *inode)
{
	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
	...
	f2fs_info(sbi, "[sis-statistic]: inode[%lu] removed, dec hidden[%lu] link, nlink = %d",
		inode->i_ino, hidden->i_ino, hidden->i_nlink);
	f2fs_sis_unlink(hidden);

	trace_f2fs_sis_dec_hidden_link(inode, hidden);
	put_hidden_inode(hidden);
}
#endif
...
int f2fs_remove_inode_page(struct inode *inode)
{
	...
	if (f2fs_is_dummy_inode(inode))
		f2fs_sis_dec_hidden_link(inode);
#endif
	return 0;
}
