				     ...
				     unsigned int offs)
{
	...
	unsigned int i;
	int err;
	struct inode *h_inode = get_hidden_inode(inode);
	...
	for (i = offs; i < offs + len; i += blocksize, lblk_num++) {
		err = fscrypt_decrypt_block_inplace(h_inode ? : inode, page,
				blocksize, i, lblk_num);
		if (err) {
			put_hidden_inode(h_inode);
			return err;
		}
...
/*
 * copy from fscrypt_decrypt_bio, but may use hidden inode
 */
static void f2fs_fscrypt_decrypt_bio(struct bio *bio)
{
	struct bio_vec *bv;
	struct bvec_iter_all iter_all;
...
#ifdef CONFIG_F2FS_FS_SIS_DISK
		f2fs_fscrypt_decrypt_bio(ctx->bio);
#else
...
static void f2fs_trace_read_submit_io(struct bio *bio)
{
	...
	if (bio && (bio->bi_io_vec != NULL) &&
		(trace_android_fs_dataread_start_enabled() || blk_is_topio())) {
...
static void f2fs_read_end_io(struct bio *bio)
{
	...
	if (ctx && (ctx->enabled_steps & STEP_READ_ACCOUNT))
		f2fs_sis_dec_transferred_read(ctx->inode);
#endif
	if (bio->bi_status) {
						...
						int create, int flag)
{
	...
	if (flag != F2FS_GET_BLOCK_DEFAULT &&
			f2fs_is_dummy_inode(inode)) {
		/* dummy inode should not creat block */
		if (create) {
...
#ifdef CONFIG_F2FS_FS_SIS_DISK
	if (hidden) {
		trace_f2fs_sis_map_blocks(dummy, hidden);
		inode = dummy;
...
#ifdef CONFIG_F2FS_FS_SIS_DISK
static int f2fs_sis_transfer_bio(struct inode *inode, struct bio *bio)
{
	struct bio_post_read_ctx *ctx = bio->bi_private;
...
#ifdef CONFIG_F2FS_FS_SIS_DISK
		if (f2fs_sis_inode_support(F2FS_I_SB(inode), inode)) {
			ret = f2fs_sis_transfer_bio(inode, bio);
			if (ret) {
				bio_put(bio);
				bio = NULL;
				goto out;
			}
			f2fs_sis_inc_transferred_read(inode);
		}
#endif
	}
	/*
...
#ifdef CONFIG_F2FS_FS_SIS_DISK
	struct inode *hidden = NULL, *dummy = NULL, *prev = NULL;
#endif
#ifdef CONFIG_BLK_CGROUP_IOSMART
...
read_single_page:
		...
		hidden = get_hidden_inode(inode);
		if (hidden) {
			trace_f2fs_sis_map_readpage(inode, hidden);
			dummy = inode;
			inode = hidden;
		}

		/*
		 * If the inode do recover or dedup during read,
		...
		f2fs_sis_dec_transferred_read(inode);
#endif
		if (ret) {
#ifdef CONFIG_F2FS_FS_COMPRESSION
set_error_page:
...
bool f2fs_should_update_outplace(struct inode *inode, struct f2fs_io_info *fio)
{
	...
	if (f2fs_is_sis_inode(inode) &&
		is_inode_flag_set(inode, FI_RECOVER_INODE))
		return true;
#endif
#ifdef CONFIG_F2FS_FS_COMPRESSION_OPTM
...
readd:
			...
			if (f2fs_compressed_file(inode)) {
				...
				if (unlikely(f2fs_cp_error(sbi))) {
#ifdef CONFIG_F2FS_FS_COMPRESSION_OPTM
					f2fs_compress_err("cp error, ino %lu", inode->i_ino);
#endif
					goto lock_page;
				}
				...
				if (ret2 < 0) {
					...
					!f2fs_all_cluster_page_ready_slowpath(&cc,
					&pvec, i, nr_pages, end, tag, mapping)))) {
#else
					!f2fs_all_cluster_page_ready(&cc,
						&pvec, i, nr_pages, false))) {
#endif
		...
		struct page **pagep, void **fsdata)
{
	 ...
	 */
	if (file && trace_android_fs_datawrite_start_enabled() && !IS_NOQUOTA(inode)) {
		char *path, pathbuf[MAX_TRACE_PATHBUF_LEN];
...
#ifdef CONFIG_F2FS_FS_SIS_DISK
	if (!dio->write)
		f2fs_sis_dec_transferred_read(dio->inode);
#endif
				...
				sector_t *span)
{
	...
	if (f2fs_is_sis_inode(inode)) {
		f2fs_err(F2FS_I_SB(inode),
			"Swapfile not supported in sis mode");
		return -EOPNOTSUPP;
	}
#endif
