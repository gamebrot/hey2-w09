...
// Honor add for judge if data path have set xattr, eg, factory reset,
// first upgrade with userdata, g_is_data_have_attr is false.
// But OTA upgrade and normal reboot is true
#define DATA_MEDIA_PATH "/data/media"
static bool g_is_data_have_attr = false;
static bool is_data_media_path(const char *path)
{
    if (strcmp(path, DATA_MEDIA_PATH) == 0) {
        return true;
    }
    return false;
}
...
static bool check_context_match_for_dir(const char *pathname, struct dir_hash_node **new_node,
    ...
    *new_node = NULL;
    ...
    // honor add for rom upgrade
    if (is_data_media_path(pathname) && read_size == SHA1_HASH_SIZE) {
        g_is_data_have_attr = true;
    }
    return false;
}
    ...
    while ((ftsent = fts_read(fts)) != NULL) {
        ...
        case FTS_D:
            ...
            if (setrestoreconlast) {
                ...
                // honor add for rom upgrade, the /data/media/0's xattr may diff with new calc sha1.
                // when enter here, xattr is not match with calc sha1, if factory reset, there is no xattr,
                // but if rom update, the xattr have value, but may diff with new calc sha1, so set the new
                // value and skip the sub dir.
                if (new_node != NULL && is_data_media_path(ftsent->fts_path) == true && g_is_data_have_attr == true) {
                    (void)setxattr(ftsent->fts_path, RESTORECON_PARTIAL_MATCH_DIGEST, new_node->digest,
                                    SHA1_HASH_SIZE, 0);
                    selinux_log(SELINUX_ERROR, "SELinux: rom update skipping restorecon on directory(%s)\n",
                                ftsent->fts_path);
                    fts_set(fts, ftsent, FTS_SKIP);
                    continue;
                }
            ...
            /* fall through */
        default:
            ret = restorecon_sb(ftsent->fts_path, ftsent->fts_statp, nochange, verbose, seinfo, uid);
            // honor add, when Android version OTA upgrade, appdata path maybe fail, but not affect.
            // Do not skip setxattr below. the path /data/data/com.xx.xx label in installd.
            if (ret != 0 && !is_app_data_path(ftsent->fts_path)) {
                error |= ret;
            }
            break;
        }
