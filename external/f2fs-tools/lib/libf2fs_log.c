...
#if defined(WITH_FSCK) && defined(__aarch64__)
#include <imonitor.h>
#endif
#define COMP_BD_ID		928005005
#define ERROR_INFO_SIZE 512
#define NOTES_SIZE 256
int g_sis_addr_reported_inode = 0;
int g_compr_addr_reported_inode = 0;
...
#if defined(WITH_FSCK) && defined(__aarch64__)
void compr_dmd_report_send(const char* info, const char* phen)
{
	struct imonitor_eventobj *obj = NULL;
	unsigned int ret = 0;

	obj = imonitor_create_eventobj(COMP_BD_ID);
	if (obj == NULL) {
		KLOGE("fsck: cannot create the eventobj\n");
		return;
	}

	ret |= (unsigned int) imonitor_set_param_string_v2(obj, "CONTENT", info);
	ret |= (unsigned int) imonitor_set_param_string_v2(obj, "FAULT_PHENOMENON", phen);
	if (ret) {
		KLOGE("fsck, set param fail\n");
		goto out_imonitor;
	}

	if (imonitor_send_event(obj) < 0) {
		KLOGE("fsck: send event fail\n");
	}
...
void dmd_sis_err_report(unsigned long ino, int err, int ftype, const char* func, const char* fmt, ...)
{
	...
	if (err == SIS_DUMMY_ADDR_NEED_CLEAR) {
		/* blkaddr_err report once per inode */
		if (g_sis_addr_reported_inode == ino) {
			return;
		} else {
			g_sis_addr_reported_inode = ino;
		}
	}

	va_start(args, fmt);
	ret = vsnprintf(notes, NOTES_SIZE - 1, fmt, args);
	va_end(args);
	if (ret < 0) {
		KLOGE("fmt err\n");
		return;
	...
	if (ftype == F2FS_FT_SIS_HIDDEN)
		snprintf(info, sizeof(info), "errno:%d, source:%s, hidden:%lu, notes:%s", err, func, ino, notes);
	else
		snprintf(info, sizeof(info), "errno:%d, source:%s, inode1:%lu, notes:%s", err, func, ino, notes);

	snprintf(phen, sizeof(phen), "sis fsck");

	compr_dmd_report_send(info, phen);
}

void dmd_compress_err_report(unsigned long ino, int err, const char* func, const char* fmt, ...)
{
	char info[ERROR_INFO_SIZE] = {0};
	char phen[ERROR_INFO_SIZE] = {0};
	char notes[NOTES_SIZE] = {0};
	...
	if (err == COMPRESS_ADDR_ERR) {
		/* blkaddr_err report once per inode */
		if (g_compr_addr_reported_inode == ino) {
			return;
		} else {
			g_compr_addr_reported_inode = ino;
		}
	}
	...
	snprintf(phen, sizeof(phen), "compress fsck");
	snprintf(info, sizeof(info), "errno:%d, source:%s, inode:%lu, notes:%s", err, func, ino, notes);
